...
export const createContact = async(req,res)=>{
    try{
        ...
        if(contact)
            return res.status(201).json({"Message":"Contact is created","Contact":contact});
        else
            return res.status(500).json({"Error":"Internal Server Error"});
...
export const updateContact = async(req,res)=>{
    try{
        ...
        const contact = await Contacts.findByIdAndUpdate({_id:id},{
            name,
            age,
            email,
            contactNo
        ...
        if(contact)
            return res.status(201).json({"Message":"Contact is updated","Contact":contact});
        else
            return res.status(500).json({"Error":"Internal Server Error"});
...
export const deleteContact = async(req,res)=>{
    try{
        ...
        if(contact)
            return res.status(201).json({"Message":"Contact is deleted","Contact":contact});
        else
            return res.status(500).json({"Error":"Internal Server Error"});
